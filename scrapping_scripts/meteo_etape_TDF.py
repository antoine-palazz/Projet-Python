...
def scrape_weather(year, stage):
    ...
    return (
        weather_search[0].split(":")[1].strip() if weather_search else "Non disponible"
    )


# Lecture du fichier CSV
df = pd.read_excel(
    "C:/Users/Utilisateur/Documents/cours ENSAE/2A/1_semestre/projet_datascience_python/tour_de_france/dates_etapes.xlsx"
)


# Scraping de la météo pour chaque étape
df["Weather"] = df.apply(
    lambda row: scrape_weather(str(row["annee"]), str(row["numero_etape"])), axis=1
)
